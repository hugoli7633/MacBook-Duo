...
导出器复用 `GlassMetalView.shader`、`HingeDemoMotion` 和 `EffectAngle`，以 90° 为起始模糊角度。设备外观与示例壁纸均裁自工程现有 Apple 产品图，来源与版权见 [素材说明](../Assets/SOURCES.md)。不读取屏幕录制权限，不捕获个人桌面。Core Image 用于离线合成机身透视，因此这不是 SwiftUI 设置窗口的逐帧录屏。
